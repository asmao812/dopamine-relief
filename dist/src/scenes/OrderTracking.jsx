...
function OrderTracking({ cartItems = [], savedAmount = 0, orderId, onGoHome }) {
    ...
    const initMap = () => {
      ...
      try {
        ...
        const startMarker = new window.AMap.Marker({
          ...
          label: {
            content: '<div style="background:#533afd;color:#fff;padding:2px 8px;border-radius:10px;font-size:11px;white-space:nowrap">商家</div>',
        ...
        const riderMarker = new window.AMap.Marker({
          ...
          icon: new window.AMap.Icon({
            ...
            image: 'data:image/svg+xml,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 40 40"><circle cx="20" cy="20" r="18" fill="#533afd" stroke="#fff" stroke-width="2"/><text x="20" y="26" text-anchor="middle" font-size="18">🛵</text></svg>'),
            imageSize: new window.AMap.Size(32, 32),
          }),
          anchor: 'center',
          zIndex: 100,
          label: {
            content: '<div style="background:#533afd;color:#fff;padding:2px 8px;border-radius:10px;font-size:10px;white-space:nowrap;animation:pulse 1.5s infinite">🏍️ 骑手</div>',
  ...
  return (
    <div className="animate-in" style={{ paddingBottom: 40 }}>
      {/* 顶部订单编号 */}
      <div style={{
        padding: '16px', background: 'linear-gradient(135deg, #1c1e54, #533afd)',
